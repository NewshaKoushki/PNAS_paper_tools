...
%% Graph1 YAP & RMST vs time
fig1 = figure;

for i = 1:size(Nuc2Cyto(pos,:),2)
    
    % plot on the left
    plot([0:i-1]*time_interval,Nuc2Cyto(pos,1:i),'yo','MarkerFaceColor','y','markers',3);
    h1 = gca;
    set(h1, 'YAxisLocation', 'left', 'xlim', get(h1, 'xlim'));
    xlabel('time (min)','fontsize',16);
    ylabel( 'YAP intensity nucleus/cytoplasm','fontsize',16); 
    xlim([0 (size(Nuc2Cyto(pos,:),2)-1)*time_interval]);
    ylim([0 1.5]);
    h1 = gca;
 ...
 hold
    ...
    pause (0.5)
    frame = getframe(1);
    util.write_gif(util.gen_addr([pathnameMat '\' 'pos' num2str(pos) '_Nuc2CytovsRMST.gif']), 0.5 + (i == 1)*1.5, frame.cdata);
    
end

 frame1 = getframe(1);
export_fig(frame1,util.gen_addr([pathnameMat '\' 'pos' num2str(pos) '_Plot_Nuc2CytovsRMST' '.tif']),'-transparent');

%% Graph2 YAP & Strain Energy vs time
fig2 = figure;
...
for i = 1:size(Nuc2Cyto(pos,:),2)
 ...
 hold
    ...
    pause (0.5)
    frame = getframe(1);
    util.write_gif(util.gen_addr([pathnameMat '\' 'pos' num2str(pos) '_Nuc2CytovsStrainEnergy.gif']), 0.5 + (i == 1)*1.5, frame.cdata);
    
end

frame2 = getframe(1);
export_fig(frame2,util.gen_addr([pathnameMat '\' 'pos' num2str(pos) '_Plot_Nuc2CytovsStrain' '.tif']),'-transparent');
%%