function [imgout, npos]=ci_loadLif(filename,number)
...
%% Added by Matin     
    if dimensions(n).DimID==4
        dimensions(n).DimID=3;
    end
    
    dimensions(n).DimID
% ed of added by Matin     
...
for n=1:nTags
    ...
    tag_rank_list(tagCount) = tagRank;
    ...
    if tagRank~=1
        if tagRank~=tagList{tagCount-1,1};
            tagRankList=tag_rank_list(1:tagCount);
