...
function [rmst_iterative,max_traction, theta0, Trace_moment, Uecm, sumforce, prestress, area_cell ,mean_displacement] = ...
...
for   cellno = 1:ncells
    ...
    set(gcf,'Position',rect,'Renderer','zbuffer'); clf; hold on;
     ...
     caxis([0 8]) % This the scale bar for the displacement (microns)
...
%     
    ...
    tot_force   = sum(sum(sqrt(tx(cell).^2 + ty(cell).^2)))%*spacing^2 probabely wrong commented on Sep 29 2017 by Rosa ;
    ...
    mean_displacement = sqrt(mean2(ux(cell).^2 + uy(cell).^2)); %average displacement over the cell area
    ...
    caxis([0 1000]) % This is the scale bar for constrained traction (Pa)
