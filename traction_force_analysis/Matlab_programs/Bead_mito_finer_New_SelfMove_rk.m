...
%----------------------------------------------------------------------------------------------------------
image_directory = strcat('C:\C2C12\croppeddata\',cellfolder);
...
%   Obtain displacements by comparing the very first image of cycle with trypsin image (displacement from Iva's program)
...
%   Obtain deltaX and deltaY summed over prescribed no_of_points to reduce time pts where traction is to be evaluated
%   The deltaX and deltaY obtained at points prescribed above is summed with the displacements from trypsinized image
    ...
    DY = [];
     ...
     for i = 1: floor(length(time_buffer)/no_of_points)
         ...
         for j = 1: length(x_mito),
              ...
              X               = num2str(x_mito_selected(j));
              Y               = num2str(y_mito_selected(j));
             m(j,:)           = [x_mito_selected(j),y_mito_selected(j),(DX(i,j)+DX_trypsin(j)),(DY(i,j)+DY_trypsin(j))];
%       m(j,:)           = [x_mito_selected(j),y_mito_selected(j),(DX(i,j)),(DY(i,j))];
end; % j loop

%       header = 'Summed displacement at corresponding time point comparing with trypsized image, with synchronous motion'
%       colnames = {'X','Y','DeltaX','DeltaY'};
        header = '';
        colnames = '';
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    	...
    	for i = 1:number,
            ...
            %time1(i) = (i)*0.32;  
            sinus(i) = sin(2*pi*time1(i)*frequency_hz*Freq_k+pi);
            cosinus(i) = cos(2*pi*time1(i)*frequency_hz*Freq_k+pi);  
...
%                           
         ...
         for j = 1: length(x_mito),
              ...
              X               = num2str(x_mito_selected(j));
              Y               = num2str(y_mito_selected(j));
%            m1(j,:)           = [x_mito_selected(j),y_mito_selected(j),(DX1(i,j)),(DY1(i,j))];
            m1(j,:)           = [x_mito_selected(j),y_mito_selected(j),(DX1(i,j)+DX_trypsin(j)),(DY1(i,j)+DY_trypsin(j))];
           m2(j,:)           = [x_mito_selected(j),y_mito_selected(j),(DX2(i,j)),(DY2(i,j))];
%            m2(j,:)           = [x_mito_selected(j),y_mito_selected(j),(DX2(i,j)),(DY2(i,j))];
        end; % j loop

%       header = 'Summed displacement at corresponding time point comparing with trypsized image, with ONLY sine and without sine'
%       colnames = {'X','Y','DeltaX','DeltaY'};
        header = '';
        colnames = '';
...
%     Add a leading zero to time points with 4 digits (so that windows arranges files correcly by name)
        ...
        xlsfilename1 = strcat('trypsindisp_NoSine_',num2str(first),'_',timestr,'.xls');
        xlsfilename2 = strcat('trypsindisp_OnlySine_',num2str(first),'_',timestr,'.xls');
      cd(save_directory);
      
      disp('store NoSine data as txt-file');
      xlswrite(m1,header,colnames,xlsfilename1);
      
       disp('store OnlySine data as txt-file');
      xlswrite(m2,header,colnames,xlsfilename2);
