...
%%%%%%%%
  ...
  ncells=2  %%%%this is what I changed
%====================================================================================
...
[a_sym, lambda, ~] = rotate1([Dxx Dxy; Dyx Dyy]);
...
%   xlabel('x (\mum)','FontSize',18,'FontWeight','bold'); ylabel('y (\mum)','FontSize',18,'FontWeight','bold');
 ...
 traction_tot(kk,:,:)=sqrt(tx.^2+ty.^2)*1e12;
 save('traction','traction_tot')
 
 surf(x,y,sqrt(tx.^2+ty.^2)*1e12); 
%   view(2); colormap jet; shading faceted; 
   view(2); colormap jet; shading interp; 
   cbh = colorbar; set(cbh,'FontSize',18,'FontWeight','bold');%caxis([0, 500]);
