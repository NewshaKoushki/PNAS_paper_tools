...
for fi1 = 1 : length(folds),
    ...
    cd ([foldAdr '/rawdata' '/' cellfolder '/Mark_and_Find_002']); pwd;
    d2tryp = dir;
    si = 0;
    ...
    %      cd( [foldAdr '/croppeddata/' cellfolder '/BF']); pwd,
    %stretchinput;
    %
    ...
    %     case 'Yes',
    %
    %         cd([foldAdr '/uniaxialstretch/croppeddata'  '/' cellfolder  ]); pwd;
    %
    d2 = dir; si = 1;
    ...
    for bound = 3: length(d2b),
        ...
        phase = strcat([foldAdr '/croppeddata/',cellfolder,'/BigBF/',phasetemp(1:end-9),'_Ch02.tif']);
