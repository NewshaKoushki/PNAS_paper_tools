...
folds         = {'Cell57'};
pixelsize     = 0.462; %0.92 for Rama's Inverted 10x ; 1.297 for Xinyong 10x; 0.462 for Bart - 20x 
 young        = 26000; %Pa
 pois         = 0.48;
 %ncells       = 3;
...
eval(['cd C:\Singlecell\traction\' cellfolder]); 
      str1 = 'Pos     Side    t       Cell    RMST    MT      CM            F    F(Unconst)    Area  L(1,1) L(2,2) Theta';
...
eval(['cd C:\Singlecell\displacement\' cellfolder]); pwd;
   ...
   load(filename);
   xrub1 = (xrub - mean(displ(:,1))) * pixelsize;
   yrub1 = (yrub - mean(displ(:,2))) * pixelsize;
 
area_cell = polyarea(xrub1,yrub1);

cell2 = find(xv >= min(xvorig) & xv <= max(xvorig) &...
             yv >= min(yvorig) & yv <= max(yvorig));
cell3  = inpolygon(xv(cell2),yv(cell2),xrub1,yrub1);
cell3  = cell2(find(cell3 > 0));       % POINTS IN THE INTERIOR OF THE CELL

%====================================================================================
%To find totalforce using unconstrained traction

tot_force_unconst   = sum(sum(sqrt(tx(cell3).^2 + ty(cell3).^2)))*spacing^2;
% 
% %====================================================================================
...
%tot_force_unconst   = sum(sum(sqrt(tx(cell).^2 + ty(cell).^2)))*spacing^2;
...
%         h = quiver3(x(1:2:232, 1:2:232),y (1:2:232, 1:2:232),m3*ones(size(x)/2),tx(1:2:232, 1:2:232),ty(1:2:232, 1:2:232),zeros(size(x)/2),1);
    set(h,'Color','w','LineWidth',1);
      axis image; 
      axis tight; 
      axis ij;
...
% Entries in traction file are: 1. RMS traction, 2. Median traction, 3. CM, 4. Total force, 5. Total force unconst, and 5. Cell area      
tdata          = [Pos, Side, time, cellno, rmst_iterative*1e12,median_trac*1e12,-Trace_moment*1e12,tot_force*1e9,tot_force_unconst*1e9, area_cell,-lambda(1,1)*1e12,-lambda(2,2)*1e12,theta*180/pi];
