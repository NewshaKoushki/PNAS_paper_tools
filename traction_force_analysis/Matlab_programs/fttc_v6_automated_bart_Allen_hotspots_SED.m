...
folds         = {'Cell3103' };
pixelsize     = 0.2522; %0.92 for Rama's Inverted 10x ; 1.297 for Xinyong 10x; 0.462 for Bart - 20x; 0.2522 for Allen dynamic; 0.3027 for Allen Static
 young        = 4000; %Pa !!!! change if 26000Pa
 pois         = 0.48;
 ncells       = 1;
...
eval(['cd C:\Singlecell\traction\' cellfolder]); 
      str1 = 'Pos     Side    t       Cell    RMST    MT      CM            F    Area  L(1,1) L(2,2) Theta   SED';
...
%  Save figures parameters  
     ...
     hscumutitle = strcat('Hotspots Cumulative (counts)');
...
% Save tractions to a file

  str2 = 'x        y      tx      ty   ';
     fName1 = strcat('C:\Singlecell\traction\', cellfolder, '\',filename(1:18),'.dat');
...
% % %traction(cell,:)           = [x(cell),y(cell),tx(cell),ty(cell)];
% % for txi = 1:size(x,1)
% %     for txj = 1:size(x,2)
% %         traction=[x(txi,txj),y(txi,txj),tx(txi,txj)*1e12,ty(txi,txj)*1e12];
% %         dlmwrite(fName1,traction,'-append',...  %# Print the matrix
% %          'delimiter','\t',...
% %          'newline','pc');
% %     end;
% % end;
...
%    figure(4);
%    cmap = contrast(figure(3));
% colormap(cmap);
%    
%  Save the constrained traction figure to a frame 


% % % CREATE HOTSPOTS MAPS
% % 
% % 
% % Traction_Map = sqrt(tx.^2+ty.^2)*1e12; %map of traction amplitude
% % 
% % traction_mean = mean(mean(Traction_Map(cell)));
% % 
% % idx_hotspots = find(Traction_Map>traction_mean*2); %find the pxl of high traction=hotspots
% % 
% % Hotspots_Map = zeros(size(Traction_Map)); %create a hotspots map
% % Hotspots_Map(idx_hotspots) = 1; %
% % 
% % %save(strcat('HS', savefilename(1:digit-1),'_',num2str(cellno),'.mat'), 'Hotspots_Map')
% % 
% % 
% % if exist('Hotspots_Cum','var') == 0,
% %     Hotspots_Cum = zeros(size(Traction_Map));   % initialise the variable at the 1st step of the loop
% % end
% % 
% % if exist('Traction_Cum','var') == 0,
% %     Traction_Cum = zeros(size(Traction_Map));   % initialise the variable at the 1st step of the loop
% % end
% % 
% % Hotspots_Cum = Hotspots_Cum + Hotspots_Map; %cumulation of hotspots in time => stationarity
% % Traction_Cum = Traction_Cum + Traction_Map; %should be way more noisy but informative
...
%       set(gcf,'Position',rect,'Renderer','zbuffer'); clf; hold on;
% %      set(gcf,'color',[1 1 1],'Position',rect,'Renderer','zbuffer'); clf; hold on;
%      set(figure(4),'Colormap',mycmap);
% %    set(gca,'color',[0 0 0]);
%   
% 
%    %caxis([0 3000])
%    surf(x,y,Hotspots_Map);
%    whitebg('black');
% 
%    %  view(2); colormap jet; shading interp; 
%      view(2); shading interp;
...
tdata          = [Pos, Side, time, cellno, rmst_iterative*1e12,median_trac*1e12,-Trace_moment*1e12,tot_force*1e9,area_cell,-lambda(1,1)*1e12,-lambda(2,2)*1e12,theta*180/pi, Uecm*1e12];
...
% % %SAVE HOTSPOTS CUMULATIVE MAP
% % 
% % save('Hotspots_Cum.mat', 'Hotspots_Cum')
% % 
% % figure(5);
% %    rect = [100, 100, 800, 560]; 
% %     load('MyColormaps','mycmap')
% %       set(gcf,'Position',rect,'Renderer','zbuffer'); clf; hold on;
% % %      set(gcf,'color',[1 1 1],'Position',rect,'Renderer','zbuffer'); clf; hold on;
% %      set(figure(5),'Colormap',mycmap);
% % %    set(gca,'color',[0 0 0]);
% %   
% % 
% %    caxis([0 145])
% %    surf(x,y,Hotspots_Cum,'EdgeColor','none');
% %    whitebg('black');
% %    
% %    
% %    
...
% %   xlabel('x (\mum)','FontSize',18,'FontWeight','bold'); ylabel('y (\mum)','FontSize',18,'FontWeight','bold');
% % tractime = str2num(savefilename(14:16))*10;
% %     headertrac = strcat(hscumutitle,';',savefilename(1:6));
% %    title(hscumutitle,'FontSize',18,'FontWeight','bold');   
% %    
% %    
% % saveas(gcf, 'Hotspots_Cum.tif');
% %    
% %    
% % %SAVE TRACTION CUMULATIVE MAP
% % 
% % save('Traction_Cum.mat', 'Traction_Cum')
% % 
% % figure(6);
% %    rect = [100, 100, 800, 560]; 
% %     load('MyColormaps','mycmap')
% %       set(gcf,'Position',rect,'Renderer','zbuffer'); clf; hold on;
% % %      set(gcf,'color',[1 1 1],'Position',rect,'Renderer','zbuffer'); clf; hold on;
% %      set(figure(6),'Colormap',mycmap);
% % %    set(gca,'color',[0 0 0]);
% %   
% % 
% %    %caxis([0 145])
% %    surf(x,y,Traction_Cum,'EdgeColor','none');
% %    whitebg('black');
% %    
% %    
% %    
...
% %   xlabel('x (\mum)','FontSize',18,'FontWeight','bold'); ylabel('y (\mum)','FontSize',18,'FontWeight','bold');
% % tractime = str2num(savefilename(14:16))*10;
% %     headertrac = strcat('Traction Cumulative',';',savefilename(1:6));
% %    title('Traction Cumulative','FontSize',18,'FontWeight','bold');   
% %    
% %    
% % saveas(gcf, 'Traction_Cum.tif');



   
%clear a* A* B* c* C* d* D* h* H* i* j* k* m* n* p* r* s* T* t* u* U* v* x* y*;
%clear Hotspots_Cum Traction_Cum
close all;
end
