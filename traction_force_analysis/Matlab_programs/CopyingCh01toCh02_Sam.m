...
% channel 
fdate = 'rawdata';
folds  = {'Cell149'}; % cell folder to analyze
...
for fi1 = 1: length(folds)      
    ...
    if  ~isempty(findstr(d2(i).name,'ch01')),	% name must  contain ch01
        copyfile(d2(i).name, strcat(d2(i).name(1:20),num2str(2),d2(i).name(22:25)));
    ...
    if  ~isempty(findstr(d2tryp(j).name,'ch01')),	% name must  contain Pos
        copyfile(d2tryp(j).name, strcat(d2tryp(j).name(1:20),num2str(2),d2tryp(j).name(22:25)));
    end;    
    end;
