...
for t=1:time_points, 
    ...
    % Create a big TH matrix that gathers info about the Tension of Hotspots 
    if exist('TH_SumUp','var') == 0, % initialise the variable at the 1st step of the loop
    TH_SumUp = zeros(length(x),time_points);  
    end
    
    % Fill in the SumUp matrix with Hotspots (1) or not (0) 
    Tmax = max(T_SumUp(:,t)); 
    idx = find(T_SumUp(:,t)>1000); % defines the threshold for hotspots definition
    H_SumUp(idx,t) = 1;
    TH_SumUp(idx,t) = T_SumUp(idx,t);
   
end %loop over time points 

% PART FOR REGULAR histogram
H_Cum_v = sum(H_SumUp,2);
T_Cum_v = sum(T_SumUp,2);
TH_Cum_v = sum(TH_SumUp,2);
idx_zeros = find (H_Cum_v == 0);

H_Cum_v(idx_zeros) = NaN;
T_Cum_v(idx_zeros) = NaN;
TH_Cum_v(idx_zeros) = NaN;
bin_time = (2.5:5:142.5);
hist_cum = hist(H_Cum_v,bin_time);


save('histo_cum.mat','bin_time','hist_cum')
save('scatter.mat', 'H_Cum_v','T_Cum_v','TH_Cum_v')

clear T_SumUp H_SumUp TH_SumUp
