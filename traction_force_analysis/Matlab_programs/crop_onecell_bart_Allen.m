...
fdate = 'rawdata';
folds  = {'Cell111'}; % cell folder to analyze 
...
%%mkdir(strcat('\HTSbart\croppeddata\',cellfolder,'\BigFB'));
    ...
    for posn = 1 : pos,% CHANGED THIS TO GET VALUES FROM FIRST ROW OF DISHES
        ...
        if  posn <= 9, 
        FB = strcat('Pos00',num2str(posn),'_S001_t99_ch01.tif');
        BigFB = strcat('Pos00',num2str(posn),'_S001_t99_ch00.tif');
        BF = strcat('Pos00',num2str(posn),'_S001_t99_ch02.tif');
        else
        FB = strcat('Pos0',num2str(posn),'_S001_t99_ch01.tif');
        BigFB = strcat('Pos0',num2str(posn),'_S001_t99_ch00.tif');
  ...
  %      im1FB_S002      = im1FB(i3:i4,j3:j4);
   ...
   %     im1BigFB_S002   = im1BigFB(i3:i4,j3:j4); 
        ...
        if  posn <= 9,
                  if time <= 10,
        name2 = strcat('Pos00',num2str(posn),'_S001_t0',num2str(time-1),'_ch01.tif')
                  else
                      name2 = strcat('Pos00',num2str(posn),'_S001_t',num2str(time-1),'_ch01.tif')
                  end;
        else
            if time <= 10,
        name2 = strcat('Pos0',num2str(posn),'_S001_t0',num2str(time-1),'_ch01.tif')
                  else
                      name2 = strcat('Pos0',num2str(posn),'_S001_t',num2str(time-1),'_ch01.tif')
                  end;
...
%            
       % CROP THE BF IMAGES:
       if posn <= 9,
        name4 = strcat('Pos00',num2str(posn),'_S001_t0',num2str(time-1),'_ch00.tif');
           else
        name4 = strcat('Pos0',num2str(posn),'_S001_t0',num2str(time-1),'_ch00.tif'); 
       end;
