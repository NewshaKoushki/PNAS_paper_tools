...
folds         = {'Cell0001'};
foldAdr = '/Users/Rose/Dropbox/Postdoc/Algorithms/Singlecell';
...
for fi1 = 1: length(folds)
    ...
    for fi1 = 1: length(folds)
        ...
        cd ([foldAdr '/traction/' cellfolder]);
        str1 = 'Pos      t       Cell    RMST       CM            Area  L(1,1)          L(2,2)           Theta       StrainEnergy(pJ)';
        ...
        savedirectory = strcat('/Users/Rose/Dropbox/Postdoc/Algorithms/Singlecell/traction/',cellfolder,'/');
        %xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
        ...
        for d2i = 3 : length(d2),
            if ~isempty(findstr(d2(d2i).name,'.dat')) &  ...		% search for an excel file
                    ~isempty(findstr(d2(d2i).name,'Position')), 			% name has to contain 'trypsindisp'
                d2b(si+2)	=d2(d2i);
        ...
        for kloop = 3: length(d2b),
            ...
            for d2idisp = 3 : length(d2disp),
                if ~isempty(findstr(d2disp(d2idisp).name,'.mat')) &  ...		% search for a mat file
                        ~isempty(findstr(d2disp(d2idisp).name,savefilename(1:32))), 			% name has to contain particular string
            ...
            for   cellno = 1 : ncells,
                ...
                cd( [foldAdr '/displacement/' cellfolder]); pwd;
                ...
                caxis([0 350]) % This is the scale bar for constrained traction (Pa)
                surf(x,y,sqrt(tx.^2+ty.^2)*1e12);
                whitebg('black');
                ...
                Pos      = str2num(savefilename(26:28));
                time     = str2num(savefilename(31:32));
