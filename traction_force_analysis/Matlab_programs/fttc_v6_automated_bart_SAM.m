...
eval(['cd C:\Singlecell\traction\' cellfolder]); 
      str1 = 'Pos     Side    t       Cell    RMST    MT      CM            F    Area  L(1,1) L(2,2) Theta  ForcePolarity';
...
% % of cell boundary files in the ../Displacements folder for each image
   ...
   for d2idisp = 3 : length(d2disp),
      if ~isempty(findstr(d2disp(d2idisp).name,'.mat')) &  ...		% search for a mat file
          ~isempty(findstr(d2disp(d2idisp).name,savefilename(1:16))), 			% name has to contain particular string
...
%    set(gca,'color',[0 0 0]);
  ...
  % caxis([0 3000]) % SAM SCALING TRACTION
   surf(x,y,sqrt(tx.^2+ty.^2)*1e12);
   whitebg('black');
...
headertrac = strcat(tractitle,';',filename(1:18));
...
tdata          = [Pos, Side, time, cellno, rmst_iterative*1e12,median_trac*1e12,-Trace_moment*1e12,tot_force*1e9,area_cell,-lambda(1,1)*1e12,-lambda(2,2)*1e12,theta*180/pi,lambda(1,1)/lambda(2,2)];
