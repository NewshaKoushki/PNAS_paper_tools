...
folds         = {'Cell1002'};
pixelsize     = 0.231; %0.92 for Rama's Inverted 10x ; 1.297 for Xinyong 10x; 0.462 for Bart - 20x 
 young        = 26000; %Pa
 pois         = 0.48;
 %ncells       = 3;
...
eval(['cd C:\Singlecell\traction\' cellfolder]); 
      str1 = 'Pos     Side    t       Cell    RMST(cell)    RMST(pt1)    RMST(pt2)   RMST(pt3)  RMST(pt4)   RMST(pt5)   RMST(pt6)  RMST(pt7)    RMST(pt8)   MT      CM            F    Area  L(1,1) L(2,2) Theta';
...
% of cell boundary files in the ../Displacements folder for each image
   ...
   for d2idisp = 3 : length(d2disp),
      if ~isempty(findstr(d2disp(d2idisp).name,'1.mat')) &  ...		% search for a mat file
...
eval(['cd C:\Singlecell\displacement\' cellfolder]); pwd;
   ...
   filename2 = strcat(savefilename(1:digit-1),'_',num2str(2),'.mat');
   filename3 = strcat(savefilename(1:digit-1),'_',num2str(3),'.mat');
   filename4 = strcat(savefilename(1:digit-1),'_',num2str(4),'.mat');
   filename5 = strcat(savefilename(1:digit-1),'_',num2str(5),'.mat');
   filename6 = strcat(savefilename(1:digit-1),'_',num2str(6),'.mat');
   filename7 = strcat(savefilename(1:digit-1),'_',num2str(7),'.mat');
   filename8 = strcat(savefilename(1:digit-1),'_',num2str(8),'.mat');
   filename9 = strcat(savefilename(1:digit-1),'_',num2str(9),'.mat');
   ...
   yrub2 = (yrub2 - mean(displ(:,2))) * pixelsize;
      ...
      load(filename4);
   xrub3 = (xrub3 - mean(displ(:,1))) * pixelsize;
   yrub3 = (yrub3 - mean(displ(:,2))) * pixelsize;
   
      load(filename5);
   xrub4 = (xrub4 - mean(displ(:,1))) * pixelsize;
   yrub4 = (yrub4 - mean(displ(:,2))) * pixelsize;
   
      load(filename6);
   xrub5 = (xrub5 - mean(displ(:,1))) * pixelsize;
   yrub5 = (yrub5 - mean(displ(:,2))) * pixelsize;
   
      load(filename7);
   xrub6 = (xrub6 - mean(displ(:,1))) * pixelsize;
   yrub6 = (yrub6 - mean(displ(:,2))) * pixelsize;
   
      load(filename8);
   xrub7 = (xrub7 - mean(displ(:,1))) * pixelsize;
   yrub7 = (yrub7 - mean(displ(:,2))) * pixelsize;
   
      load(filename9);
   xrub8 = (xrub8 - mean(displ(:,1))) * pixelsize;
   yrub8 = (yrub8 - mean(displ(:,2))) * pixelsize;
...
% Calculate the RMS traction in a rectangular region around the first selected point

% Find x,y points of grid closest to position of magnetic bead
[diffx,Ix] =min(abs(xrub1(1)-x(1,:)));
[diffy,Iy] =min(abs(yrub1(1)-y(:,1)));

% Choose only those points in the rectangular region that are within the cell boundary

cell2 = find(xv >= x(1,Ix-1) & xv <= x(1,Ix+1) &...
             yv >= y(Iy-1,1) & yv <= y(Iy+1,1));
cell3  = inpolygon(xv(cell2),yv(cell2),xrub,yrub);
cell3  = cell2(find(cell3 > 0));       % POINTS IN THE INTERIOR OF THE RECTANGULAR BOUNDARY

 rmst_region = sqrt(mean2(tx(cell3).^2 + ty(cell3).^2));

     
 Xs = [x(1,Ix-1) x(1,Ix-1) x(1,Ix+1) x(1,Ix+1) x(1,Ix-1)];
 Ys = [y(Iy-1,1) y(Iy+1,1) y(Iy+1,1) y(Iy-1,1) y(Iy-1,1)];

%====================================================================================

% Calculate the RMS traction in a rectangular region around the second selected point

% Find x,y points of grid closest to selected point
[diffx,Ix] =min(abs(xrub2(1)-x(1,:)));
[diffy,Iy] =min(abs(yrub2(1)-y(:,1)));
...
cell3  = cell2(find(cell3 > 0));       % POINTS IN THE INTERIOR OF THE RECTANGULAR BOUNDARY
 ...
 rmst_region2 = sqrt(mean2(tx(cell3).^2 + ty(cell3).^2));

     
 Xs2 = [x(1,Ix-1) x(1,Ix-1) x(1,Ix+1) x(1,Ix+1) x(1,Ix-1)];
 Ys2 = [y(Iy-1,1) y(Iy+1,1) y(Iy+1,1) y(Iy-1,1) y(Iy-1,1)];
...
% Find x,y points of grid closest to position of magnetic bead
[diffx,Ix] =min(abs(xrub3(1)-x(1,:)));
[diffy,Iy] =min(abs(yrub3(1)-y(:,1)));
...
cell3  = cell2(find(cell3 > 0));       % POINTS IN THE INTERIOR OF THE RECTANGULAR BOUNDARY
 ...
 rmst_region3 = sqrt(mean2(tx(cell3).^2 + ty(cell3).^2));

     
 Xs3 = [x(1,Ix-1) x(1,Ix-1) x(1,Ix+1) x(1,Ix+1) x(1,Ix-1)];
 Ys3 = [y(Iy-1,1) y(Iy+1,1) y(Iy+1,1) y(Iy-1,1) y(Iy-1,1)];
...
% Find x,y points of grid closest to selected point
[diffx,Ix] =min(abs(xrub4(1)-x(1,:)));
[diffy,Iy] =min(abs(yrub4(1)-y(:,1)));
...
cell3  = cell2(find(cell3 > 0));       % POINTS IN THE INTERIOR OF THE RECTANGULAR BOUNDARY
 ...
 rmst_region4 = sqrt(mean2(tx(cell3).^2 + ty(cell3).^2));

     
 Xs4 = [x(1,Ix-1) x(1,Ix-1) x(1,Ix+1) x(1,Ix+1) x(1,Ix-1)];
 Ys4 = [y(Iy-1,1) y(Iy+1,1) y(Iy+1,1) y(Iy-1,1) y(Iy-1,1)];

%====================================================================================
% Calculate the RMS traction in a rectangular region around the second selected point

% Find x,y points of grid closest to selected point
[diffx,Ix] =min(abs(xrub5(1)-x(1,:)));
[diffy,Iy] =min(abs(yrub5(1)-y(:,1)));
...
cell3  = cell2(find(cell3 > 0));       % POINTS IN THE INTERIOR OF THE RECTANGULAR BOUNDARY
 ...
 rmst_region5 = sqrt(mean2(tx(cell3).^2 + ty(cell3).^2));

     
 Xs5 = [x(1,Ix-1) x(1,Ix-1) x(1,Ix+1) x(1,Ix+1) x(1,Ix-1)];
 Ys5 = [y(Iy-1,1) y(Iy+1,1) y(Iy+1,1) y(Iy-1,1) y(Iy-1,1)];
...
% Find x,y points of grid closest to selected point
[diffx,Ix] =min(abs(xrub6(1)-x(1,:)));
[diffy,Iy] =min(abs(yrub6(1)-y(:,1)));
...
cell3  = cell2(find(cell3 > 0));       % POINTS IN THE INTERIOR OF THE RECTANGULAR BOUNDARY
 ...
 rmst_region6 = sqrt(mean2(tx(cell3).^2 + ty(cell3).^2));

     
 Xs6 = [x(1,Ix-1) x(1,Ix-1) x(1,Ix+1) x(1,Ix+1) x(1,Ix-1)];
 Ys6 = [y(Iy-1,1) y(Iy+1,1) y(Iy+1,1) y(Iy-1,1) y(Iy-1,1)];
...
% Find x,y points of grid closest to selected point
[diffx,Ix] =min(abs(xrub7(1)-x(1,:)));
[diffy,Iy] =min(abs(yrub7(1)-y(:,1)));
...
cell3  = cell2(find(cell3 > 0));       % POINTS IN THE INTERIOR OF THE RECTANGULAR BOUNDARY
 ...
 rmst_region7 = sqrt(mean2(tx(cell3).^2 + ty(cell3).^2));

     
 Xs7 = [x(1,Ix-1) x(1,Ix-1) x(1,Ix+1) x(1,Ix+1) x(1,Ix-1)];
 Ys7 = [y(Iy-1,1) y(Iy+1,1) y(Iy+1,1) y(Iy-1,1) y(Iy-1,1)];
...
% Find x,y points of grid closest to selected point
[diffx,Ix] =min(abs(xrub8(1)-x(1,:)));
[diffy,Iy] =min(abs(yrub8(1)-y(:,1)));
...
cell3  = cell2(find(cell3 > 0));       % POINTS IN THE INTERIOR OF THE RECTANGULAR BOUNDARY
 ...
 rmst_region8 = sqrt(mean2(tx(cell3).^2 + ty(cell3).^2));

     
 Xs8 = [x(1,Ix-1) x(1,Ix-1) x(1,Ix+1) x(1,Ix+1) x(1,Ix-1)];
 Ys8 = [y(Iy-1,1) y(Iy+1,1) y(Iy+1,1) y(Iy-1,1) y(Iy-1,1)];
...
%     view(2); shading flat;
   ...
   h = plot3(xrub,yrub,m3*ones(size(xrub)),'w-');
    set(h,'LineWidth',2.0);
      h = quiver3(x,y,m3*ones(size(x)),tx,ty,zeros(size(x)),1, 'MaxHeadSize', 1, 'MarkerSize', 12);
    set(h,'Color','w','LineWidth',1);
      axis image; 
      axis tight; 
      axis ij;
...
%   plot3(xrub1,yrub1,m3*ones(size(xrub1)),'r.'); 
% 
%   plot3(Xs2,Ys2,m3*ones(size(Xs2)),'r-');
%   plot3(xrub2,yrub2,m3*ones(size(xrub2)),'ro');
%   
%   plot3(Xs3,Ys3,m3*ones(size(Xs3)),'r-');
%   plot3(xrub3,yrub3,m3*ones(size(xrub3)),'rx');
%   
%   plot3(Xs4,Ys4,m3*ones(size(Xs4)),'r-');
%   plot3(xrub4,yrub4,m3*ones(size(xrub4)),'r+');
%   
%   plot3(Xs5,Ys5,m3*ones(size(Xs5)),'r-');
%   plot3(xrub5,yrub5,m3*ones(size(xrub5)),'r*');
%   
%   plot3(Xs6,Ys6,m3*ones(size(Xs6)),'r-');
%   plot3(xrub6,yrub6,m3*ones(size(xrub6)),'rs');
%   
%   plot3(Xs7,Ys7,m3*ones(size(Xs7)),'r-');
%   plot3(xrub7,yrub7,m3*ones(size(xrub7)),'rd');
%   
%   plot3(Xs8,Ys8,m3*ones(size(Xs8)),'r-');
%   plot3(xrub8,yrub8,m3*ones(size(xrub8)),'rv');
...
tdata          = [Pos, Side, time, cellno, rmst_iterative*1e12,rmst_region*1e12,rmst_region2*1e12,rmst_region3*1e12,rmst_region4*1e12, ...
    rmst_region5*1e12,rmst_region6*1e12,rmst_region7*1e12,rmst_region8*1e12,median_trac*1e12,-Trace_moment*1e12, ...
    tot_force*1e9,area_cell,-lambda(1,1)*1e12,-lambda(2,2)*1e12,theta*180/pi];
