...
folds         = {'Cell0813'};
pixelsize     = 0.252; %0.92 for Rama's Inverted 10x ; 1.297 for Xinyong 10x; 0.462 for Bart - 20x 
 young        = 9000; %Pa
 pois         = 0.48;
 %ncells       = 3;
...
eval(['cd C:\Singlecell\traction\' cellfolder]); 
      str1 = 'Pos     Side    t       Cell    RMST    MT      CM            F    Area  L(1,1) L(2,2) Theta   StrainEnergy(pJ)';
...
% Settings for an animated movie
     ...
     aviobj1 = VideoWriter(mvfile1,'Motion JPEG AVI');
     aviobj2 = VideoWriter(mvfile2,'Motion JPEG AVI');
     aviobj3 = VideoWriter(mvfile3,'Motion JPEG AVI');
