...
function puppy_code(prop, x, y, dx , dy, xrub, yrub)
...
young_e12         = young * 1e-12; % why? (pooya wonders)
...
%         num2str(disptime),'min');
    ...
    while step <= iteration && are_the_same == 0,
        ...
        if step > 3,
            if abs(maxt(step) - maxt(step-1)) <= the_same% * maxt(step) % POOYA MESS
                are_the_same = 1;
            end
        end
        step = step + 1;
    ...
    caxis([0 2000]) % This is the scale bar for constrained traction (Pa)
    surf(x,y,sqrt(tx.^2+ty.^2)*1e12);
    whitebg('black');
...
%         num2str(tractime),'min');
    title('traction','FontSize',18,'FontWeight','bold'); % headertrac
...
%     POOYA MESS: search for savefilename and fix it
%     Pos      = str2num(savefilename(26:28)); % POOYA MESS
%     time     = str2num(savefilename(31:32)); % POOYA MESS
    Pos = 0;
    time = 0;
