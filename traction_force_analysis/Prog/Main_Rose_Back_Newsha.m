...
clear
%% Settings
boundary_drawing_step_iteration = 5;
crop_size = 25 * 32 - 1;

prop.pixelsize = 0.22727; %0.22727; % 0.37879; % size of each pixel in micron
prop.young = 5000;
...
for task = 1:2 % task = 1 means user input selections, task = 2 means processing
    ...
    if task == 1
       ...
       imFB_org_ts = im_ts.Image{1};
        imBigFB_ts = im_ts.Image{3};
        imBF_ts = im_ts.Image{3};
        
%         im_fin = lif_load(filename, npos/2 + pos);
im_fin = lif_load(filename, 24+ pos);
        imFB_fin_org = im_fin.Image{1}(:,:,1);
       % imBigFB_fin_org =im_fin.Image{4}(:,:,1); %im_fin.Image{1};
        %imBF_fin_org = im_fin.Image{4}(:,:,1);
       ...
       % imBF_fin = imcrop(imBF_fin_org, rect(pos,:));
        ...
        for ts = [1:num_ts]
            ...
            [rmst_iterative,max_traction, theta0, Trace_moment, Uecm, sumforce, prestress,...
                area_cell] = puppy_code(prop, x, y, dx , dy, xrub{pos,ts}, yrub{pos,ts}, Pos_folder, ts);
...
save([ResFolder '.mat'], 'rmst', 'max_rmst', 'orientation', 'netmoment', 'StrainEnergy', 'MaxCumForce', 'Area');
